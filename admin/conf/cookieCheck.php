...
$__adminkey = trim(isset($_COOKIE["adminkey"]) ? $_COOKIE["adminkey"] : "" );
...
$__t = 0;
if (isset($_COOKIE["t"]) && trim($_COOKIE["t"]) != "") {
...
if ($___adminkey != $__adminkey || $___t > 1800) {
    ...
    $_uid = trim(isset($_COOKIE["uuid"]) ? $_COOKIE["uuid"] : "");
    $uuid = '';
    if ($_uid != '') {
        $uuid = str_decrypt_base64($_uid);
    }
    // var_dump( $uuid);
    if (strlen($uuid) > 32) {
