...
class DBManage {
    ...
    function backup($tablename = '', $dir, $size) {
        ...
        if (!empty($tablename)) {
            ...
            // 文件名前面部分  
            $filename = date('YmdHis') . '_#' . mt_rand(10000, 99999) . "_" . $tablename;
            // 字段数量  
            ...
            $sql .= $this->_retrieve();
            // 文件名前面部分  
            $filename = date('YmdHis') . '_#' . mt_rand(10000, 99999) . "_all";
            // 查出所有表  
...
//                var_dump(serialize($record [$i]));
//                var_dump($record [$i]);
//                var_dump($i);
                $insert .= ($comma . "NULL");
            } else {
                $insert .= ($comma . "'" . mysql_real_escape_string($v) . "'");
            }
            $comma = ",";
        }
        $insert .= ");" . $this->ds;
